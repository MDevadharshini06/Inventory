...
if os.environ.get('FLASK_APP_REEXEC') != '1':
    try:
        import importlib
        ...
        if os.path.exists(venv_py):
            ...
            os.environ['FLASK_APP_REEXEC'] = '1'
            print('Re-executing with virtualenv python:', venv_py)
...
with app.app_context():
    ...
    try:
        with db.engine.connect() as conn:
            ...
            res2 = conn.execute(text("PRAGMA table_info(movement)"))
            existing_m = [row[1] for row in res2.fetchall()]
            if 'timestamp' not in existing_m:
                
                try:
                    conn.execute(text("ALTER TABLE movement ADD COLUMN timestamp DATETIME DEFAULT (CURRENT_TIMESTAMP)"))
                except Exception:
                    
                    conn.execute(text("ALTER TABLE movement ADD COLUMN timestamp DATETIME"))
...
@app.route('/movements')
def movements():
    moves = Movement.query.order_by(Movement.id.desc()).all()
    movement_list = []
    for m in moves:
        movement_list.append({
            'id': m.id,
            'product_name': get_product_name(m.product_id),
            'from_location': get_location_name(m.from_location_id),
            'to_location': get_location_name(m.to_location_id),
            'qty': m.qty,
...
def add_movement():
    if request.method == 'POST':
        ...
        if not from_input and not to_input:
            flash("Enter at least one of From or To location.")
            return redirect(url_for('add_movement'))
        if qty <= 0:
        ...
        move = Movement(product_id=product_id, from_location_id=from_loc, to_location_id=to_loc, qty=qty)
        db.session.add(move)
        db.session.commit()
...
def report():
    ...
    for p in products:
        if getattr(p, 'location_id', None):
            ...
            balances[key] = balances.get(key, 0) + (getattr(p, 'quantity', 0) or 0)

    moves = Movement.query.order_by(Movement.id).all()
    for m in moves:
    ...
    return render_template('report.html', report=report_rows)
